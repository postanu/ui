...
import { timeRef } from '../../composables/timeRef'
import { useNow } from '../../composables/useNow'
import PInput from '../p-input/PInput.vue'

type InputType = 'h' | 'm' | 'ampm'
type InputRef = null | typeof PInput
...
export default defineComponent({
	...
	setup (props, { emit }) {
		let { locales, modelValue } = toRefs(props)

		let { hours, minutes, ampm } = timeRef(
			modelValue,
			locales.value,
			newDate => {
		...
		let focus = ref<false | InputType>(false)

		let ampmInput = ref<InputRef>(null)
		let hoursInput = ref<InputRef>(null)
		let minutesInput = ref<InputRef>(null)

		let isMilitary = computed(() => ampm.value === null)
